...
(defun starup-game/opening ()
  (message  "『2015又是一幅好光景啊』加班结束之后的你看了看四周，已是深夜。")
  (message  "你握紧了拳头，心想『我不能再这么加班下去了，我要改变世界』，灌木丛里的野狗叫了一声，以示鼓励。")
  (starup-game-util/next)
  (message  "你决定一起开发一款屌炸天的应用，叫做：")
  (setq starup-game-project (make-project))
  (message "你决定给公司起名为：")
  (setq starup-game-company (make-company))
  (message "你成立了%s" (company-name starup-game-company))
  (message "你拿出了你毕生的积蓄%s作为%s的启动资金。"
           (company-money starup-game-company)
           (project-name starup-game-project))
  (message "点子和钱都有了，就差几个员工了")
  (starup-game-util/next))
...
(defun starup-game//mvp ()
  (message  "经过一番估计, 大家认为%s的开发难度为%s点困难度, 这可是个不小的工程，要加油干了。"
            (project-name starup-game-project)
  ...
  (if (> (company-money starup-game-company) 0)
      (progn
        (message "%s的第一版终于撸出来啦，他已经具备基本功能" (project-name starup-game-project))
        (starup-game/beta))
    (message "资金耗尽，项目失败...")
      ))
...
(defun starup-game/work-one-week ()
  ...
  (let ((old-remain-difficulty (project-diffcultiy starup-game-project)))
    (message  "第%d周开始了，键盘的敲击声响起" starup-game-week)
    ...
    (let* ((remain-difficulty (project-diffcultiy starup-game-project))
      ...
      (message "第%d周结束了，成功完成了%d点困难点，还剩下%d困难点等待开发"
               starup-game-week
               forward
               remain-difficulty))))
...
(defun starup-game/staff ()
  (let* ((coders (load-coders))
         (coder-names (mapcar (lambda (coder)
                                (oref coder name))
                              coders)))
    (message "Many thanks to :%s" (string-join code-names ","))))
